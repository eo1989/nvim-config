...
cmd.abbrev('<buffer> != ~=')
cmd.inoreabbrev('<buffer> null nil')
cmd.inoreabbrev('<buffer> none nil')
cmd.inoreabbrev('<buffer> None nil')

-- vim.opt.path:append(',' .. vim.fn.expand('config') .. '/**')
-- vim.opt.path:append(',' .. vim.fn.expand('data') .. '/**')
-- vim.opt_local.suffixesadd:prepend('.lua')
-- vim.opt_local.suffixesadd:prepend('init.lua')

-- vim.opt.inc = [[\v<((do|load)file|require)\s*\(?['"]\zs[^'"]+\ze['"]]

-- use this after you create the needed functions from eo_nvim/nvim/after/ftplugin/lua.lua
    ...
    surround.buffer_setup {
      surrounds = {
        ...
        F = {
          add = function()
            return {
              { fmt('function %s() ', get_input('Enter a function name: ')) },
              { ' end' },
            }
          end,
        },
