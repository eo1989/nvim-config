...
function eo.adjust_split_height(min_height, max_height)
...
local function is_list_open(list_type)
  return vim.iter(fn.getwininfo()):find(function(win) return not eo.falsy(win[list_type]) end) ~= nil
end
...
function eo.list.qf.delete(buf)
...
---@return string
function eo.truncate(str, max_len)
  assert(str and max_len, 'string and max_len must be provided')
  return api.nvim_strwidth(str) > max_len and str:sub(1, max_len) .. eo.ui.icons.misc.ellipsis or str
end

---Determine if a value of any type is empty
---@param item any
---@return boolean?
function eo.falsy(item)
...
function eo.ftplugin_conf(configs)
  if type(configs) ~= 'table' then return end
  for name, callback in pairs(configs) do
    local ok, plugin = eo.pcall(require, name)
...
function eo.filetype_settings(map)
    ...
    return {
      ...
      command = function(args)
        vim.iter(settings):each(function(key, value)
          ...
          if key == 'plugins' then return eo.ftplugin_conf(value) end
          if type(key) == 'function' then return eo.pcall(key, args) end
          vim.iter(value):each(function(option, setting) vim[key][option] = setting end)
        end)
      end,
    }
  end)
  eo.augroup('filetype-settings', unpack(commands:totable()))
end
...
---@param opts table?
function eo.command(name, rhs, opts)
  opts = opts or {}
...
---@return boolean
function eo.has(feature) return fn.has(feature) > 0 end
...
function eo.reqidx(require_path)
  return setmetatable({}, {
...
function eo.reqcall(require_path)
  return setmetatable({}, {
