...
local root_names = { '.git', 'MakeFile', 'go.mod', 'go.sum', 'pyproject.toml', 'requirements.txt' }
...
local function set_root(args)
  ...
  if not root or path then return end
...
eo.augroup('FindProjectRoot', { event = 'BufEnter', command = set_root })
